...
| Method | Path | Description |
|---|---|---|
| GET | `/api/v1/counsellor-chart/students/{studentId}` | **Student-or-staff**, ownership-guarded (a student may only read their own chart; staff may read any). Assemble the full chart: `ourChampion` (profile), `academicRecord`, `preCounselling` (4 sections of student-vs-parent parameter rows), `assessment` (the computed report), `flaggedMirrorPairs` (strong/gap-0 contradictions only), `entranceExamsTable`/`collegesTable` (computed, read-only — see below), and `counsellor` (saved notes/SCRI/ratings/roadmap grid/career DNA narrative/why-this-stream text). It's all the student's own data — no different from what `POST /accept` already hands back to them in full once the chart is finalized — this just makes the same read available earlier too. For MCQ_SINGLE/MCQ_MULTI/MATRIX pre-counselling questions, `parameters[].student`/`.parent` are resolved against `FormQuestion.options` — option labels, not raw stored codes (a MATRIX sub-field is resolved against that sub-field's own options). Lazily creates an empty chart row if none exists. 404 if the student doesn't exist. |
| PUT | `/api/v1/counsellor-chart/students/{studentId}` | Partial save of counsellor-authored content. Body (all optional): `notes` (`[{ code: "A1".."H4", body }]`, ≤10 lines each), `scri` (`{ confidence, reasonedThinking, reducedAnxiety, selfAwareness, careerCuriosity, decisionOwnership }`, each 1–4 — band recomputed), `academicTrend`, `alignmentRating`, `strengths`/`hobbies`/`careerShortlist`, `roadmapGrid` (9 plain strings: `nowSkills, nowActivities, nowHabits, c11Stream, c11Exams, c11Electives, afterDegrees, afterCertifications, afterAbroad`), `careerDnaNarrative` (5 plain strings: `dnaDefinition, careerStyleReveals, personalityStyleReveals, thinkingModeReveals, aptitudeProfileReveals`), `whyThisStream` (`{ whyThisStream1, whyThisStream2 }`), `lastEditedBy`. These three groups are free-text pass-throughs — no scoring or cross-validation. `entranceExamsTable`/`collegesTable` are **not** accepted here — like Graduation Fit, they're computed on GET, not counsellor-editable (see below). A save carrying **real content** advances the student's workflow to `COUNSELLOR_FEEDBACK_REPORT` (a save with only `lastEditedBy` does not). |
...
`entranceExamsTable`/`collegesTable` (arrays of `{ id, fullName, conductingBody, level, applicableFor, subjectRequirements, examMonth, urlLink }` / `{ id, collegeName, location, type, course, entranceExam, ranking, website }`) are computed from `assessment.careerFit.top3Industries` — not `graduationPathways` (its `mainStream`/`subStream` taxonomy has no live link into the Career Library) — by resolving each top industry to its `CareerIndustry` row and pulling ACTIVE `EntranceExam`s linked to that industry's job roles and ACTIVE `Institution`s linked to the industry directly, deduped across the top 3. Empty until the student has a submitted assessment with library data to score `careerFit` against (`src/modules/counsellor-chart/entrance-exams-colleges.ts`).
