...
| Method | Path | Description |
|---|---|---|
| GET | `/api/v1/counsellor-chart/students/{studentId}` | Assemble the full chart: `ourChampion` (profile), `academicRecord`, `preCounselling` (4 sections of student-vs-parent parameter rows), `assessment` (the computed report), `flaggedMirrorPairs` (strong/gap-0 contradictions only), `entranceExamsTable`/`collegesTable` (computed, read-only — see below), and `counsellor` (saved notes/SCRI/ratings/roadmap grid/career DNA narrative/why-this-stream text). For MCQ_SINGLE/MCQ_MULTI/MATRIX pre-counselling questions, `parameters[].student`/`.parent` are resolved against `FormQuestion.options` — option labels, not raw stored codes (a MATRIX sub-field is resolved against that sub-field's own options). Lazily creates an empty chart row if none exists. 404 if the student doesn't exist. |
