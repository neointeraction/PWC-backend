...
| POST | `/api/v1/counsellor-chart/students/{studentId}/accept` | **Student-or-staff**, ownership-guarded (a student may only accept their own chart; staff may call it for any student). The student's acknowledgement of the finalized chart — stamps `acceptedAt` (returned as `counsellor.acceptedAt`, alongside `counsellor.finalizedAt`). Distinct from `finalizedAt`: that's the counsellor's "I'm done" stamp, this is the student's own. **This is what gates the chart's career-library job-role proposals into the Super Admin's pending queue** — see `GET /career-library/proposals` above. **Idempotent** — re-accepting keeps the original timestamp. 400 if the chart hasn't been finalized yet. |
