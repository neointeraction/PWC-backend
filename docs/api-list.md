...
| GET | `/api/v1/sessions` | Admin oversight list. Query: `projectId?, studentId?, counsellorId?, status?, from?, to?` (date range on `scheduledDate`), `noShow?` (`STUDENT`\|`COUNSELLOR` — filters to sessions where that party's no-show flag is set; the operational-metric read for the monthly availability review). Every session response (this and every other session read/write endpoint) nests `student: { id, studentCode, mobile, whatsappNumber, parentEmail, parentMobile, user }` — `whatsappNumber` falls back to `mobile` when the student has no separate WhatsApp number recorded, rather than coming back `null`. |
