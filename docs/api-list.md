...
Last updated: 2026-09-15 (`POST /api/v1/projects/wizard` now requires at least one
student row — an empty roster is rejected with 400).
...
| POST | `/api/v1/projects/wizard` | **Combined "Finish" call for the create-project wizard** — creates the project, onboards its student roster, and imports its counsellor-availability sheet, all in **one transaction** (a failure partway through rolls back everything, including the project itself). Body: `{ project, students, counsellorSlots? }`. `project` is exactly the `POST /projects` body. `students[]` is the `POST /students` body minus `projectId` (implied) — **required, at least one row** (400 otherwise); a project can't be created with an empty roster. `counsellorSlots[]` is one row per bookable slot: `{ counsellorCode, firstName?, lastName?, email?, mobile?, meetingLink?, date, startTime, endTime }`, keyed by `counsellorCode` (not a Prisma id) — rows sharing a code are the same counsellor's slots. If that `counsellorCode` already exists it's matched and assigned to the project as-is (identity fields on the row are ignored); if it's new, **at least one row for that code** must carry `firstName`/`lastName`/`email`/`mobile` so a Counsellor + User can be created inline (400 otherwise). Sends the same `LOGIN_CREDENTIALS_STUDENT` email per student as the standalone `POST /students`, but only after the transaction commits — `PRE_COUNSELLING_PARENT` still waits for each student's own `confirm-profile` call, same as the standalone flow. Response: `{ project, studentsCreated, counsellorsAssigned, slotsImported }`. 409 on any unique-field collision (duplicate project `name`/`contactNumber`/`primaryEmail`/`code`, student `email`/`mobile`/`studentCode`, etc — the whole call rolls back). Admin only. `counsellorSlots` is optional — onboarding counsellors individually afterward (via their own endpoints) still works — but `students` is not, since an empty roster is disallowed. |
...
| GET | `/api/v1/career-library/institutions` | **Typeahead dropdown.** Canonical institutions/colleges. Query: `search?`, `domainId?`, `limit?`. Institutions are shared at the **industry** level (see `POST /career-library` below) — `domainId` scopes to institutions already linked via that domain's industry; 400 if it isn't a live domain. Add `status?` (`DRAFT`\|`ACTIVE`, default `ACTIVE`) for the admin review queue — pickers show active rows only. Each row carries `status` + `submittedBy`. |
| GET | `/api/v1/career-library/courses` | **Typeahead dropdown.** Canonical courses. Query: `search?`, `level?`, `domainId?`, `limit?`. Courses are shared at the **cluster** level (see `POST /career-library` below) — `domainId` scopes to courses already linked via that domain's cluster; 400 if it isn't a live domain. Add `status?` (`DRAFT`\|`ACTIVE`, default `ACTIVE`) for the admin review queue — pickers show active rows only. Each row carries `status` + `submittedBy`. |
