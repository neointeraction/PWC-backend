...
- Phone numbers (`mobile`, `whatsappNumber`, `parentMobile`, `contactNumber`, ...): all go
  through the shared `phoneSchema` (`src/common/validators/shared.ts`), which strips a
  leading `+91`/`0091` before storing — so `+919876543210` and `9876543210` are stored and
  compared as the same value, `9876543210`. Non-Indian numbers are left as sent.

Last updated: 2026-09-21 (`POST /api/v1/sessions` — admin manual creation — now also fires the
`SESSION_SCHEDULED_CONFIRMATION_*` emails, as self-service booking always did; the three templates
gained an optional `sessionNumber`). Earlier the same day (`PATCH /api/v1/students/me` now also accepts `firstName`/`lastName`
— eleven fields — applied only while the student is still `DRAFT`, ignored afterwards). Earlier,
2026-09-20 (`POST /api/v1/projects/wizard` and the shared error handler no
longer answer bad input with a 500: impossible slot/project dates, NUL characters and
malformed JSON are now 400s, an oversized body is 413, transient DB timeouts are 503, and a
new counsellor whose email already belongs to any user is skipped rather than 409ing the
call — see the wizard entry below). Earlier, 2026-09-16: the wizard no longer rolls back
the whole call on one bad student row or one booked counsellor slot.
...
| POST | `/api/v1/projects/wizard` | **Combined "Finish" call for the create-project wizard** — creates the project, onboards its student roster, and imports its counsellor-availability sheet, in **one transaction**. `project` (code/name/contactNumber/primaryEmail) is still all-or-nothing — a conflict there rejects the whole call (409), nothing is created. `students[]` and `counsellorSlots[]` are each validated **per row inside the same transaction** and a conflicting row is **skipped, not fatal** — the rest of the roster/sheet still lands. Body: `{ project, students, counsellorSlots? }`. `project` is exactly the `POST /projects` body. `students[]` is the `POST /students` body minus `projectId` (implied) — **required, at least one row in the payload** (400 otherwise); a row whose `email`/`mobile`/`studentCode` already exists (or collides with another row in the same payload) is skipped and reported in `studentsSkipped`, and if literally every row conflicts the whole call 400s (mirrors the `students.min(1)` empty-roster case — a project can't end up with zero students). `counsellorSlots[]` is one row per bookable slot: `{ counsellorCode, firstName?, lastName?, email?, mobile?, meetingLink?, date, startTime, endTime }`, keyed by `counsellorCode` (not a Prisma id) — rows sharing a code are the same counsellor's slots. If that `counsellorCode` already exists it's matched and assigned to the project as-is (identity fields on the row are ignored); if it's new, **at least one row for that code** must carry `firstName`/`lastName`/`email`/`mobile` so a Counsellor + User can be created inline — if none does, or the identity's `email`/`mobile` is already taken, every row for that code is skipped (reported in `slotsSkipped`) instead of 400ing the call. An email that already belongs to **any** user (student/admin, not only a counsellor) or to a student in this same payload counts as "already taken" here. `date` must be a real calendar day (`2026-02-31` → 400). A slot row that collides with an existing `counsellorId`+`slotDate`+`startTime` booking (globally unique per counsellor, not per project — same constraint `addSlots`/`POST /sessions/slots/import` enforce), or duplicates another row in the same payload, is likewise skipped rather than fatal. **Counsellor project-assignment reflects only slots that actually landed** — a `counsellorCode` that resolved to a real counsellor but had every one of its slots skipped is not assigned to the project by this call. Sends the same `LOGIN_CREDENTIALS_STUDENT` email per created student as the standalone `POST /students`, but only after the transaction commits — `PRE_COUNSELLING_PARENT` still waits for each student's own `confirm-profile` call, same as the standalone flow. Response: `{ project, studentsCreated, studentsSkipped: [{ index, reason }], counsellorsAssigned, slotsImported, slotsSkipped: [{ counsellorCode, date, startTime, reason }] }` — `index` in `studentsSkipped` matches the row's position in the `students[]` array sent, same convention as `POST /students/check-duplicates`. Admin only. `counsellorSlots` is optional — onboarding counsellors individually afterward (via their own endpoints) still works — but `students` is not, since an empty roster is disallowed. |

## Students

| Method | Path | Description |
|---|---|---|
| POST | `/api/v1/students` | Create a student. Also creates a linked `User` (role `STUDENT`) with a temp password (from the `password?` body field if given, otherwise generated; `mustChangePassword` set), returned once in the response. Body: `firstName, lastName?, email, mobile, whatsappNumber?, alternateMobile?, alternateEmail?, studentCode, projectId, className, divisionName, parentMobile?, parentEmail?, fatherWhatsapp?, fatherName?, fatherOccupation?, fatherEmployer?, motherName?, motherOccupation?, motherEmployer?` — `studentCode` (e.g. `S0001`) is a **required, admin-assigned** human-readable login id, **not auto-generated**; `lastName` is **optional** (stored as `""` when omitted or `null` — send nothing rather than repeating `firstName`; names in emails/reports then render as just the first name); `className`/`divisionName` are free-text strings (no institute-owned class/division lookup); `parentMobile`/`parentEmail` are optional and not unique (siblings may share a parent's contact details) — leave them unset rather than falling back to the student's own `mobile`/`email`; the father/mother breakdown is optional (bulk imports may carry only a single parent contact); `fatherOccupation`, `motherName`, `motherOccupation` are stored as `null` when omitted (`fatherName` stored as `""`). Sends `LOGIN_CREDENTIALS_STUDENT` to the student's own email, best-effort (doesn't fail the request). `PRE_COUNSELLING_PARENT` is **not** sent here — see `POST /students/{id}/confirm-profile` below. |
| POST | `/api/v1/students/check-duplicates` | **Bulk pre-check for a file upload** (e.g. the project-creation wizard's student roster step), before any row is submitted via `POST /students`. Body: `{ students: [{ email?, studentCode?, mobile? }] }` — up to 2000 rows, each needing at least one of the three fields. Searches **across all projects** (unscoped by `projectId`), since `email`/`studentCode`/`mobile` are each globally unique (`@unique` on `User.email`/`Student.studentCode`/`Student.mobile`). Response: `{ results: [{ index, isDuplicate, matches: [{ field, value, studentId, projectId, projectName }] }] }`, one entry per input row **in the same order**, `matches` empty when `isDuplicate` is `false`. Staff only. |
| GET | `/api/v1/students` | List students, each with a computed **`stageInfo`** (see "Student stage & ageing" below). Query: `projectId?, className?, divisionName?, workflowStatus?` plus the derived-stage/ageing filters `stage?` (derived-stage dropdown key), `flagged?` (`true`/`false` — the 🚩 follow-up toggle), and `discontinued?` (`true`/`false` — filter by `isDiscontinued`; omitted = no filtering). Staff only. **A `COUNSELLOR` caller is scoped to only the students they have a booked `Session` with** — not every student in a project they're assigned to; `ADMIN`/`SUPER_ADMIN`/`VIEW_ONLY_ADMIN` still see everything the query matches. |
...
| PATCH | `/api/v1/students/me` | **Student self-service edit.** The logged-in student updates their own name and contact/parent details. Partial body, **fourteen** whitelisted fields: `firstName`, `lastName`, `whatsappNumber`, `alternateMobile`, `alternateEmail`, `parentMobile`, `parentEmail`, `fatherWhatsapp`, `fatherName`, `fatherOccupation`, `fatherEmployer`, `motherName`, `motherOccupation`, `motherEmployer`. `firstName` (trimmed, non-empty — empty/whitespace-only → 400) / `lastName` (trimmed; `""` or `null` clears it) update the linked `User` row in the same transaction, but **only while `workflowStatus` is `DRAFT`** (before `confirm-profile`); after that they are **silently ignored** (still 200, the other fields still apply — no 409/403), and a value equal to the current one is a no-op. `email`, primary `mobile`, `studentCode`, `className`/`divisionName`, `projectId`, `workflowStatus` are **not** editable here — those stay admin-only via `PATCH /students/{id}`. The other twelve fields are allowed at any workflow stage. Returns the same enriched shape as `GET /students/me` (`user.firstName`/`user.lastName` reflect the new name). 404 for a non-student account. |
| GET | `/api/v1/students/{id}` | Get one student (with user, project, `className`/`divisionName`). Includes `workflowStatus`. Staff only — students read themselves via `/students/me`. A `COUNSELLOR` caller gets a 404 (not 403, to avoid leaking existence) unless they have a booked `Session` with this student. |
...
| POST | `/api/v1/students/{id}/confirm-profile` | Student confirms **their own** profile data (father/mother details, parent contact) is correct — or staff on their behalf (a student confirming another student's profile is `403`). Advances `workflowStatus` `DRAFT → PROFILE_COMPLETED`. 409 if not currently `DRAFT`. This is what sends the parent their `PRE_COUNSELLING_PARENT` email (when `parentEmail` is set) — not student creation — with a link straight to their form (`${APP_WEB_URL}/forms/PRE_COUNSELLING_PARENT/students/{id}`); best-effort, doesn't fail the request. |
...
| `SESSION_1_COMPLETED` | Session 1: both parties joined — the second `POST /sessions/{id}/join` auto-completes it |
| `COUNSELLOR_FEEDBACK_REPORT` | `PUT /counsellor-chart/students/{id}` with real content |
| `SESSION_2_COMPLETED` | Session 2: both parties joined, same as above |
...
| GET | `/api/v1/assessment/stream-weights` | List every `StreamWeight` row (`id, mainStream, subStream, coreSubjects, electiveSubjects, explanation`), sorted by `mainStream, subStream`. `weights` (internal scoring config) is omitted. Read-only reference lookup backing the Counsellor Chart's Career Direction "Add Row" **Stream Fit** dropdown. Any authenticated user. |
| GET | `/api/v1/assessment/graduate-stream-weights` | List every `GraduateStreamWeight` row (`id, clusterHead, mainStream, subStream, specialisations, eligibility, keyExams, explanation`), sorted by `mainStream, subStream`. `weights` omitted. Backs the Career Direction "Add Row" **Graduation Fit** dropdown. Any authenticated user. |
...
| GET | `/api/v1/career-library/institutions` | **Typeahead dropdown.** Canonical institutions/colleges. Query: `search?`, `domainId?`, `limit?`. Institutions are curated per job role — `domainId` scopes to institutions already linked to job role(s) in that domain ("what this domain's own job roles already have"); 400 if it isn't a live domain. Add `status?` (`DRAFT`\|`ACTIVE`, default `ACTIVE`) for the admin review queue — pickers show active rows only. Each row carries `status` + `submittedBy`. |
| GET | `/api/v1/career-library/courses` | **Typeahead dropdown.** Canonical courses. Query: `search?`, `level?`, `domainId?`, `limit?`. Courses are curated per job role — `domainId` scopes to courses already linked to job role(s) in that domain; 400 if it isn't a live domain. Add `status?` (`DRAFT`\|`ACTIVE`, default `ACTIVE`) for the admin review queue — pickers show active rows only. Each row carries `status` + `submittedBy`. |
...
| POST | `/api/v1/career-library` | **Staff.** Create an entry. Required: `domainId` (a live `CareerDomain` leaf — cluster/industry are derived from it; 400 if unknown or soft-deleted), `jobRole, aiResilienceGrade, aiResilienceComment, oneLineDescription`. Optional: `studentId` (added from that student's Counsellor Chart — the service 404s if the student doesn't exist and derives `projectId` from `Student.projectId` server-side; omit for the standalone Career Library admin screen), salary/qualification fields (incl. `qualification10th12th` and its `qualification10th12thExplanation`, `qualificationGraduationDefined`, `qualificationPGDefined`), `roleOverview`, `keySkills` (string list), `topCompanies`, `certifications*`, `status` (default `DRAFT`, admin-only meaning — see below), and the normalized links `entranceExams` / `courses` / `institutions` / `educationEntries` (each `[{ id } \| { name, … }]`; exam items need `level` when added by name, education items are `[{ id } \| { level, programme, description? }]`). Every one of these four link types is curated **per job role** — `courses`/`institutions` link straight onto this entry alone, same as `entranceExams`/`educationEntries` (see `docs/career-library-normalization-spec.md` §10). **Two different destinations through this one route:** an admin's payload is written straight into `career_library_entries` with whatever `status` they asked for (so `status:"ACTIVE"` puts it in the library in one call), and the response is the assembled entry; a counsellor's payload never touches that table at all — it's staged as a `CareerLibraryEntryProposal` instead (`status` is ignored), and the response is the proposal (see below). Either way, a given `studentId`/`projectId` is stamped on the row so `GET /api/v1/career-library?studentId=` (once published) and `GET /api/v1/career-library/proposals?studentId=` (while pending) can re-list "job roles added from this chart". |
| PATCH | `/api/v1/career-library/{id}` | **Admin.** Partial update (any create field, incl. `status` toggle). A provided `entranceExams`/`courses`/`institutions`/`educationEntries` array **replaces that entry's own links only** — never a sibling job role's, even under the same cluster/industry; omitting an array leaves it unchanged. Removing an id from any of these four arrays also deletes that master row (`EntranceExam`/`Course`/`Institution`/`EducationEntry`) if no job role anywhere still links it. **Clearing a value:** omitting a scalar leaves it unchanged, sending `null` clears it — accepted for every nullable column (`salaryIndia*`/`salaryGlobal*` text **and** numeric, `roleOverview`, `qualification10th12thExplanation`, `qualification10th12th`, `qualificationGraduation(Defined)`, `qualificationPG(Defined)`, `entranceExamsUGDescription`). Empty strings are still rejected; clear with `null`. `jobRole`, `domainId`, `aiResilienceGrade`, `aiResilienceComment` and `oneLineDescription` are NOT NULL and reject `null`. Clear a list by sending `[]`. 400 on an unknown link `id`. Sets `updatedBy`. 404 if not found. |
| DELETE | `/api/v1/career-library/{id}` | **Admin.** Delete an entry (cascades its own entrance-exam/course/institution/education links). Any of those master rows left with no remaining link on any job role is deleted too. 404 if not found. |
| GET | `/api/v1/career-library/{id}` | Get one entry. Includes the `domain` chain (`domain.industry.cluster`), the curated normalized links `linkedEntranceExams` / `linkedCourses` / `linkedInstitutions` / `linkedEducationEntries` — all four scoped to this job role alone — plus the legacy broad value-match view `relatedInstitutions` (by `domain.industry.name`) / `relatedCourses` (by `domain.industry.cluster.name`) / `relatedEntranceExams` (kept during transition). An admin's `DRAFT` entry is visible to staff only — a student fetching one gets 404. 404 if not found. |
| GET | `/api/v1/career-library/proposals` | **Staff.** List counsellor-submitted job role proposals awaiting review. Query: `search?`, `domainId?`, `studentId?` (scope to one Counsellor Chart), `page?`, `pageSize?`. **An admin sees every proposal (the review queue); a counsellor always sees only their own** (`submittedBy` = their own user id), regardless of `studentId` — this is how a chart re-lists "job roles I already added here" on a later visit. **Gated by Session 2 completion for the admin view only:** a proposal whose `studentId` points at a student who hasn't reached `workflowStatus: SESSION_2_COMPLETED` (or later) yet is excluded from what an admin sees — it never reaches the review-queue API surface until then; a proposal with no `studentId` (submitted outside the chart flow) is never gated. The submitting counsellor's own view is never gated, so a chart can keep re-listing its own pending submissions before Session 2 has happened. Each row is hydrated with `domain` (the taxonomy chain), `linkedEntranceExams`/`linkedCourses`/`linkedInstitutions`/`linkedEducationEntries` resolved from the ids the counsellor picked, `submittedBy` (the counsellor's user id) plus `submittedByName` (resolved `"First Last"`, server-side), `projectName` (the `Student.projectId` project's name, resolved server-side; `null` if `studentId` is null), and `session2Completed` (boolean; `null` if `studentId` is null) — `studentId`/`projectId` are plain columns already present on the row. Returns `{ data, pagination }`. |
| GET | `/api/v1/career-library/proposals/{id}` | **Staff.** Get one proposal, hydrated the same way as the list. 404 if not found. |
| PATCH | `/api/v1/career-library/proposals/{id}` | **Staff.** Edit a still-pending job role proposal — same body shape as `POST /api/v1/career-library`, minus `studentId` (fixed at submission) and `status` (meaningless for a proposal). The counsellor who submitted it may edit their own; an admin may edit any of them. 403 for any other counsellor. Link arrays (`entranceExams`/`courses`/`institutions`/`educationEntries`) are re-resolved the same way as on create — a provided array replaces the proposal's own id list, omitting one leaves it unchanged. Returns the updated (hydrated) proposal. 404 if not found. |
| DELETE | `/api/v1/career-library/proposals/{id}` | **Staff.** Withdraw a still-pending job role proposal. The counsellor who submitted it may delete their own; an admin may delete any of them. 403 for any other counsellor. Returns 204. 404 if not found. |
| POST | `/api/v1/career-library/proposals/{id}/approve` | **Super Admin.** Approve a job role proposal — one level up from a regular Admin, who may still add entries straight to the library themselves. Copies it into a **new** `career_library_entries` row (fresh id, `status:ACTIVE`, `createdBy` = the original submitter, `studentId`/`projectId` carried over unchanged), materializes `CareerEntranceExam`/`CareerCourse`/`CareerInstitution`/`CareerEducationEntry` for it directly (all four scoped to this new entry alone), then deletes the proposal. Returns the assembled entry (not the proposal). 404 if the proposal id doesn't exist (either already decided, or it was never a proposal — e.g. an admin's own entry id). |
| POST | `/api/v1/career-library/proposals/{id}/reject` | **Super Admin.** Reject a job role proposal — one level up from a regular Admin. **Deletes it permanently** (no body, nothing retained) — any canonical exam/course/institution the submission created along the way stays behind, reviewed on its own via the reference-data endpoints above. Returns `{ id, deleted: true }`. 404 if missing. |
...
| POST | `/api/v1/sessions/slots` | Add slots to an existing project's inventory after the initial import (e.g. a counsellor assigned later needs availability). Body: `projectId, counsellorId, slots: [{ date, startTime, endTime }]`. A row colliding with an existing slot for that counsellor (globally unique `counsellorId`+`slotDate`+`startTime`, not per-project) or duplicated within the payload is **skipped, not fatal** — the rest of the batch still lands. Response: `{ added, skipped: [{ date, startTime }] }`. 400 if the counsellor isn't assigned to the project. Admin-only. |
| GET | `/api/v1/sessions/slots` | List counsellor slots (oversight). Query: `projectId?, counsellorId?, status?` (`OPEN`\|`BOOKED`). Nests `counsellor: { id, counsellorCode, mobile, user }`. |
| DELETE | `/api/v1/sessions/slots/:id` | Delete a single open slot. Admin-only. |
| GET | `/api/v1/sessions/students/{studentId}/booking-options` | Blind Session 1 options — deduped `{ slotDate, startTime, endTime }` list across all open slots for the student's project. Query: `sessionNumber` (`SESSION_1`\|`SESSION_2`). For `SESSION_2`, pass either `session1Date, session1StartTime` (resolves the counsellor that pick would assign — first-available, upload order — and returns only that counsellor's remaining open slots at least 2 calendar days after `session1Date`) **or** `rescheduleSessionId` (an existing `SESSION_2` belonging to this student, for rebooking it solo once Session 1 is already booked — the `session1Date`/`session1StartTime` form can't discover the counsellor at that point since Session 1's own slot is `BOOKED`, not `OPEN`; locks to that Session 2's already-assigned counsellor's other open slots, ≥2 calendar days after the student's active Session 1 — 404 if the session isn't this student's `SESSION_2`, 400 if Session 1 isn't booked yet). For `SESSION_1`, optionally pass `rescheduleSessionId` (an existing `SESSION_1` belonging to this student) to lock the preview to that session's already-assigned counsellor's other open slots (plus the 2-day gap against Session 2, if booked) instead of the blind fresh-booking list — 404 if the session isn't this student's `SESSION_1`. |
...
| POST | `/api/v1/sessions/students/{studentId}/detach` | **Admin only.** Detaches a student from their counsellor so both sessions can be reassigned — cancels Session 1 and Session 2 together (Session 2 must stay with Session 1's counsellor), releases both slots, and rolls `workflowStatus` back to `ASSESSMENT_COMPLETED`. Unlike `/restart` it works even after Session 1 has started or been `COMPLETED`, but only while `workflowStatus` is between `SESSION_SCHEDULED` and `COUNSELLOR_FEEDBACK_REPORT` (Session 2 not done yet) — 409 from `SESSION_2_COMPLETED` onward (incl. the feedback stages), if Session 2 is `COMPLETED`, or if there are no active sessions. Reactivating the cancelled rows on rebooking also clears `notes` and `dayReminderSentAt`. Doesn't touch the `CounsellorChart` (per student, not per counsellor). Sends `SESSION_CANCELLED_*` for each session cancelled. |
...
| POST | `/api/v1/sessions` | Admin manual creation for edge cases outside self-service booking — bypasses the slot inventory entirely. Body: `studentId, counsellorId, sessionNumber, date, startTime, endTime`. If the student already has a `CANCELLED` session for that `sessionNumber`, this **reactivates that row in place** (new counsellor/date/time allowed, cancellation + join/no-show fields cleared) rather than inserting a second row — `@@unique([studentId, sessionNumber])` only allows one row per session number, so this is how an admin re-books after a cancellation with a different counsellor. 409 if an existing `SCHEDULED`/`COMPLETED` session for that number is still active. **Emails** (after the row commits, best-effort — an email failure never fails the request; nothing is sent on a 400/409): `SESSION_SCHEDULED_CONFIRMATION_STUDENT` to the student, `_PARENT` to `parentEmail` (skipped if unset), `_COUNSELLOR` to the assigned counsellor — the same three `POST /sessions/students/{studentId}/book` sends. Sent for `SESSION_2` and for a reactivated `CANCELLED` row too; the copy says which session number. |
| GET | `/api/v1/sessions` | Admin oversight list. Query: `projectId?, studentId?, counsellorId?, status?, from?, to?` (date range on `scheduledDate`), `noShow?` (`STUDENT`\|`COUNSELLOR` — filters to sessions where that party's no-show flag is set; the operational-metric read for the monthly availability review). Every session response (this and every other session read/write endpoint) nests `student: { id, studentCode, mobile, whatsappNumber, parentEmail, parentMobile, user }` — `whatsappNumber` falls back to `mobile` when the student has no separate WhatsApp number recorded, rather than coming back `null`. It also nests `counsellor: { id, counsellorCode, meetingLink, mobile, user }` directly — `mobile` is included here (not just on `GET /counsellors?projectId`) so a session/slot's counsellor contact is available even for a manually-booked session whose counsellor was never formally assigned to the project via `ProjectCounsellor` and therefore wouldn't appear in that directory response. |
| GET | `/api/v1/sessions/{id}` | Get one session. Staff, or the session's own student (e.g. resolving the counsellor/report link from a report-generation page — not gated on Session 2 or Feedback). |
| POST | `/api/v1/sessions/{id}/join` | "Join Now" — records `studentJoinedAt`/`counsellorJoinedAt` and returns `meetingLink`, always the assigned counsellor's own `Counsellor.meetingLink` (no per-session link — there's no `PATCH .../meeting-link` route; set it once on the counsellor instead). Body: `{ role }` (`STUDENT`\|`COUNSELLOR`). Window: from 10 minutes before `startTime` through `endTime`; 400 outside that window. Joining `SESSION_2` 409s until the student has a `studentJoinedAt` on their `SESSION_1` row — **not** until `SESSION_1.status === "COMPLETED"`. **Whichever join is second marks the session `COMPLETED` and advances `workflowStatus`** (`SESSION_1_COMPLETED`/`SESSION_2_COMPLETED`) inline, in the same request/transaction as that join write — see `.../complete` below, which this supersedes as the normal path. A `STUDENT` join also emails the parent (`SESSION_JOINED_PARENT`); a `COUNSELLOR` join doesn't notify anyone. Completing Session 2 this way also fires the same `FEEDBACK_REQUEST_PARENT` email as `.../complete`. |
| POST | `/api/v1/sessions/{id}/complete` | Marks the session `COMPLETED` and advances `workflowStatus` to `SESSION_1_COMPLETED`/`SESSION_2_COMPLETED` — **superseded by `.../join` above as the normal trigger** (a session completes as soon as both parties join, no separate call needed); kept as a manual staff/admin correction tool. **Idempotent**: a no-op (200, session returned unchanged) if the session is already `COMPLETED`, rather than a 409. Same `SESSION_2` needs-Session-1-attended 409 as `.../join` otherwise. Completing Session 2 also emails the parent (`FEEDBACK_REQUEST_PARENT`) a link to their feedback form — parents have no login, so this is their only way in; there's no equivalent auto-send to the student, who sees the pending form in-app once logged in. |
| PATCH | `/api/v1/sessions/{id}/notes` | Counsellor adds/updates session notes — independent of the booking/join flow. Body: `{ notes }`. |
| POST | `/api/v1/sessions/{id}/reschedule` | Move to a new date/time for the same (already-locked) counsellor. Body: `{ date, startTime, initiatedBy }` (`STUDENT`\|`ADMIN` — no `COUNSELLOR` option; a counsellor who needs a session moved contacts Admin manually, who reschedules on their behalf via `ADMIN`). `STUDENT`-initiated requests are rejected within 24 hours of the current `startTime`, **and** rejected if `studentRescheduleUsed` is already true ("only 1 self-service reschedule per session" — `docs/Session Handling_Cancellation  Rescheduling.pdf` §1 Option A; contact Admin for further changes). Neither check applies when reactivating a `CANCELLED` session (its old date is no longer meaningful) or to `ADMIN`-initiated moves. Re-validates the ≥2-day gap against the student's other session — direction-aware, not just an absolute gap: `SESSION_1`'s new date must stay ≥2 days *before* `SESSION_2`'s, and `SESSION_2`'s must stay ≥2 days *after* `SESSION_1`'s, so a reschedule can't invert which physical session is chronologically first (`sessionNumber` is a fixed curriculum-stage label, never recomputed from date — see `completeSession`'s workflow-status mapping and the "Session 2 locked until Session 1 complete" rule, both keyed off the label). 400 if the move would invert the order. Releases the old slot back to `OPEN`, claims the new one. Also works on a `CANCELLED` session — reactivates it back to `SCHEDULED` (clearing `cancellationReason`/`cancellationNotes`, and resetting `studentRescheduleUsed` to false — a fresh start), still locked to the same counsellor. 409 if the session is `COMPLETED`. Sends `SESSION_RESCHEDULED_STUDENT`/`_PARENT`/`_COUNSELLOR` — the assigned counsellor is notified regardless of who initiated the move. |
...
**Note**: `meetingLink` is a static field set once on the `Counsellor` record, not
generated per session — there's no Calendly/Google Meet integration and none is
planned.
...
| Method | Path | Description |
|---|---|---|
| GET | `/api/v1/counsellor-chart/students/{studentId}` | **Student-or-staff**, ownership-guarded (a student may only read their own chart; staff may read any). Assemble the full chart: `ourChampion` (profile), `academicRecord`, `preCounselling` (4 sections of student-vs-parent parameter rows), `assessment` (the computed report), `flaggedMirrorPairs` (strong/gap-0 contradictions only), `entranceExamsTable`/`collegesTable` (counsellor-edited if saved, else computed live — see below), and `counsellor` (saved notes/SCRI/ratings/roadmap grid/career DNA narrative/why-this-stream text/the 4 other Career Direction tables). It's all the student's own data — no different from what `POST /accept` already hands back to them in full once the chart is finalized — this just makes the same read available earlier too. For MCQ_SINGLE/MCQ_MULTI/MATRIX pre-counselling questions, `parameters[].student`/`.parent` are resolved against `FormQuestion.options` — option labels, not raw stored codes (a MATRIX sub-field is resolved against that sub-field's own options). Lazily creates an empty chart row if none exists. 404 if the student doesn't exist. |
| PUT | `/api/v1/counsellor-chart/students/{studentId}` | Partial save of counsellor-authored content. Body (all optional): `notes` (`[{ code: "A1".."H4", body }]`, ≤10 lines each), `scri` (`{ confidence, reasonedThinking, reducedAnxiety, selfAwareness, careerCuriosity, decisionOwnership }`, each 1–4 — band recomputed), `academicTrend`, `alignmentRating`, `strengths`/`hobbies`/`careerShortlist`, `roadmapGrid` (9 plain strings: `nowSkills, nowActivities, nowHabits, c11Stream, c11Exams, c11Electives, afterDegrees, afterCertifications, afterAbroad`), `careerDnaNarrative` (5 plain strings: `dnaDefinition, careerStyleReveals, personalityStyleReveals, thinkingModeReveals, aptitudeProfileReveals`), `whyThisStream` (`{ whyThisStream1, whyThisStream2 }`), `lastEditedBy`, and the 6 Career Direction tables — `entranceExamsTable`, `collegesTable`, `streamFitTable`, `graduationTable`, `careerCompassClusterTable`, `careerCompassTable` (each the **entire resulting array** — system rows that survived a counsellor's delete plus any added rows; "last full array wins", no per-row diffing; an empty array is a valid "all rows deleted" state, not an omission — see below). Each row may carry `isManualEntry: true` (free-text, not picked from the career library); a manual-entry row without an `addedAt` gets one stamped with the current server time on this save, and a row that already has one keeps it — so `addedAt` reflects when that specific row was added, not the chart's last-save time (surfaced on `GET /counsellor-chart/manual-entries` above). These three text groups are free-text pass-throughs — no scoring or cross-validation. A save carrying **real content** advances the student's workflow to `COUNSELLOR_FEEDBACK_REPORT` (a save with only `lastEditedBy` does not). **409 once the student has accepted the report/chart** (`CounsellorChart.acceptedAt` set) — acceptance locks the chart from further edits, no staff override; nothing else is written on a locked save. |
...
| POST | `/api/v1/counsellor-chart/students/{studentId}/accept` | **Student-only — no staff bypass**, unlike most student self-service endpoints (ownership-guarded on top: a student may only accept their own chart). The student's acknowledgement of the finalized chart — stamps `acceptedAt` (returned as `counsellor.acceptedAt`, alongside `counsellor.finalizedAt`). Distinct from `finalizedAt`: that's the counsellor's "I'm done" stamp, this is the student's own. **This is what gates the chart's career-library job-role proposals into the Super Admin's pending queue** — see `GET /career-library/proposals` above — **and locks the chart from any further counsellor edits** (`PUT` above 409s afterward; no admin unlock). **Idempotent** — re-accepting keeps the original timestamp. 400 if the chart hasn't been finalized yet. 403 for any non-`STUDENT` caller. |
...
| GET | `/api/v1/counsellor-chart/manual-entries` | **Super Admin only.** Flat array of every free-text (`isManualEntry: true`) row a counsellor has added across the 6 Career Direction tables (below), across **all** students — `{ id, studentId, studentName, tableLabel, fields, addedBy, addedAt }`, where `tableLabel` is one of the 6 table titles verbatim, `fields` is the row's own fields minus `id`/`isManualEntry`/`addedAt`, `addedBy` is `chart.lastEditedBy`, and `addedAt` is the row's own server-stamped timestamp (see the `PUT` below) — falls back to `chart.updatedAt` only for rows written before this per-row stamp existed. No pagination. |
| DELETE | `/api/v1/counsellor-chart/manual-entries/{id}` | **Super Admin only.** The "Close" action on a manual-entry review row — `{id}` is the row's own `id` from the flat list above. Scans all 6 Career Direction tables across every student for a row with that id and `isManualEntry: true`, and removes just that row from its array (the rest of the array, and every other column, is untouched). `204` on success, `404` if the id isn't found in any student's arrays (already deleted, or a bad id). |

**Career Direction tables (Step 3 / Section C).** 6 tables, each a counsellor-edited JSON array on `CounsellorChart`: "Career Compass (Indicative Clusters)" (`careerCompassClusterTable` — add-disabled by design, delete only), "Career Compass (Target Roles & Compensation)" (`careerCompassTable`), "Assessment Result View — Stream Fit & Pathways" (`streamFitTable`), "Graduation Fit" (`graduationTable`), "Colleges After Class 11&12" (`collegesTable`), "Entrance Exams" (`entranceExamsTable`). `null`/absent (returned under `counsellor.*` for the last 4) means no counsellor edits yet; `entranceExamsTable`/`collegesTable` are the two exceptions — returned at the top level of the GET response, computed live from `assessment.careerFit.top3Industries` (not `graduationPathways` — its `mainStream`/`subStream` taxonomy has no live link into the Career Library) by resolving each top industry to its `CareerIndustry` row and pulling ACTIVE `EntranceExam`s linked to that industry's job roles and ACTIVE `Institution`s linked to the industry directly, deduped across the top 3, capped to the top 6 of each (ranked by their industry's fit-order, then alphabetically — same top-N treatment as `top6Domains`), until the counsellor's first edit is saved (`src/modules/counsellor-chart/entrance-exams-colleges.ts`); empty (not null) until the student has a submitted assessment with library data to score `careerFit` against. Item shapes: `entranceExamsTable` — `{ id, fullName, conductingBody, level, applicableFor, subjectRequirements, examMonth, urlLink, isManualEntry? }`; `collegesTable` — `{ id, collegeName, location, type, course, entranceExam, ranking, website, isManualEntry? }`; `streamFitTable` — `{ id, mainStream, subStream, coreSubjects, electives, explanation?, streamRequirement?, gradingLevel?, meaning?, isManualEntry? }`; `graduationTable` — `{ id, cluster, mainStream, subStream, specialization, reasoning, keyExams, isManualEntry? }`; `careerCompassClusterTable` — `{ id, cluster, industry, domain, streamRequirement, gradingLevel, meaning, isManualEntry? }` (this table itself stays counsellor-edited, not server-computed like the two above, but `assessment.careerFit.top3Industries` now carries a `streamRequirement` string per entry — see `docs/frontend-integration-guide.md` — for the frontend to prefill this column when seeding rows from the assessment); `careerCompassTable` — `{ id, domain, role, whyItFits, topEmployers, aiResilience, salaryIndia, salaryAbroad, isManualEntry? }`. No max-rows-per-table validation server-side — the frontend gates that.

## SCRI Band Guidance

Static reference data (4 rows, one per SCRI band) — not scoped to a student, so a
standalone lookup rather than embedded in every Counsellor Chart response. The frontend
independently computes a student's `band`/`label` from their SCRI total (see
`counsellor.scri` on the Counsellor Chart GET above); this endpoint only supplies the
guidance text per band. Any logged-in role; a good candidate for the frontend to fetch
once and cache client-side.

| Method | Path | Description |
|---|---|---|
| GET | `/api/v1/scri-band-guidance` | `{ data: [{ band, score: { min, max }, label, labelMeaning, forStudents, tipsForStudents, tipsForParent }] }`, one row per band (1–4), ordered by band. `score.min`/`score.max` are the real numeric SCRI-total cutoffs from `src/modules/counsellor-chart/scri.ts` (`SCRI_BANDS`) — band 1 is `6–10`, not `0–10`. |
...
| Method | Path | Description |
|---|---|---|
| GET | `/api/v1/reports/students/{studentId}/assessment` | The full student assessment report. Returns `student` (name, code, institute/class/division, workflowStatus), `championProfile` (DCS + DPS), `traitMap` (RIASEC / Big Five / Aptitude / Cognitive layers + flat 18-trait map), `careerCompass` (Career Fit top-6 domains with representative careers + top-3 industries — any job role a counsellor added to this student's Counsellor Chart takes a priority seat in the top-6, ahead of and bumping the lowest-priority computed fits, flagged `addedByCounsellor:true`), `streamFit`, `graduationPathways`, `reliability` (RVS/ACI/ORI/DC, plus TC and the composite ARI once every aptitude answer carries `timeTakenMs`), `counsellorNarrative` (chart strengths/hobbies/shortlist/SCRI/notes, or `null` if none authored), `feedback` (score or `{ complete:false }`), `accepted`/`acceptedAt` (the student/parent's acceptance of the report — see `POST .../accept` below), and `meta` (cohort, `assessmentSubmittedAt`, `finalized`, engine `pending` list). **404 until the student has a computed assessment result.** When the **student** fetches their own report and they are at `STUDENT_PARENT_FEEDBACK`, the case is closed (`workflowStatus` → `CLOSED`) — receiving the report is the last step. A staff fetch is a read, and never closes a case. |
| POST | `/api/v1/reports/students/{studentId}/accept` | **Student-only — no staff bypass** (403 otherwise), same as `POST /counsellor-chart/students/{id}/accept`: this is deliberately not a staff-callable action, since accepting also locks the chart. The student accepts the finalized report. Returns `{ acceptedAt }`. 404 if there's no computed assessment result yet (same "not ready" condition as the GET above); 400 if the counsellor hasn't finalized the chart yet (`POST /counsellor-chart/students/{id}/finalize`). Idempotent — re-accepting returns the original `acceptedAt` rather than erroring. Backed by the same `CounsellorChart.acceptedAt` stamp as `POST /counsellor-chart/students/{id}/accept` — accepting the report and accepting the chart are the same student action, and both routes read/write the one field, and both lock the chart. |
