...
| POST | `/api/v1/sessions/{id}/join` | "Join Now" — records `studentJoinedAt`/`counsellorJoinedAt` and returns `meetingLink`, always the assigned counsellor's own `Counsellor.meetingLink` (no per-session link — there's no `PATCH .../meeting-link` route; set it once on the counsellor instead). Body: `{ role }` (`STUDENT`\|`COUNSELLOR`). Window: from 10 minutes before `startTime` through `endTime`; 400 outside that window. Joining `SESSION_2` 409s until the student has a `studentJoinedAt` on their `SESSION_1` row — **not** until `SESSION_1.status === "COMPLETED"** (see `POST .../complete` below, which nothing in the product actually calls). A `STUDENT` join also emails the parent (`SESSION_JOINED_PARENT`); a `COUNSELLOR` join doesn't notify anyone. |
| POST | `/api/v1/sessions/{id}/complete` | Marks the session `COMPLETED` (the "Session Completed" confirmation button) — **no frontend screen actually calls this today**, so a real session stays `SCHEDULED` after it happens; treat `studentJoinedAt` as the true "did Session 1 happen" signal instead (see `.../join` above). Advances `workflowStatus` to `SESSION_1_COMPLETED` / `SESSION_2_COMPLETED`. Same `SESSION_2` needs-Session-1-attended 409 as `.../join`. Completing Session 2 also emails the parent (`FEEDBACK_REQUEST_PARENT`) a link to their feedback form — parents have no login, so this is their only way in; there's no equivalent auto-send to the student, who sees the pending form in-app once logged in. |
